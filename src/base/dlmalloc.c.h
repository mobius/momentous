 ...
 */
#if MMAP_THR_ALLOC
  FORCEINLINE void* page_alloc( void* alloc, size_t s );
  FORCEINLINE int page_dealloc( void* alloc, void* ptr, size_t s );
  #define CALL_MMAP_THR_ALLOC(alloc, s) page_alloc(alloc, s)
  #define CALL_MUNMAP_THR_ALLOC(alloc, p, s) page_dealloc(alloc, (p), (s))
#endif
...
static void* sys_alloc(mstate m, size_t nb) {
  ...
  */
  ...
  if (HAVE_MMAP && tbase == CMFAIL) {  /* Try MMAP */
    #if MMAP_THR_ALLOC
    char* mp = (char*)CALL_MMAP_THR_ALLOC(m->extp, asize);
    #else
    char* mp = (char*)(CALL_MMAP(asize));
    #endif /* MMAP_THR_ALLOC */
  ...
  if (ok_magic(ms)) {
    ...
    while (sp != 0) {
      ...
      if ((flag & USE_MMAP_BIT) && !(flag & EXTERN_BIT) &&
        #if MMAP_THR_ALLOC
          CALL_MUNMAP_THR_ALLOC(ms->extp, base, size) == 0)
        #else
      ...
      if (RTCHECK(ok_address(fm, p) && ok_inuse(p))) {
        ...
        if (!pinuse(p)) {
          ...
          if (is_mmapped(p)) {
            psize += prevsize + MMAP_FOOT_PAD;
            #if MMAP_THR_ALLOC
            if (CALL_MUNMAP_THR_ALLOC(fm->extp, (char*)p - prevsize, psize) == 0)
            #else
            if (CALL_MUNMAP((char*)p - prevsize, psize) == 0)
            #endif /* MMAP_THR_ALLOC */
              fm->footprint -= psize;
